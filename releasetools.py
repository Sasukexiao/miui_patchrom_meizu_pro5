...
def AddAssertions(info):
    ...
    for i in xrange(len(edify.script)):
        if " ||" in edify.script[i] and ("ro.product.device" in edify.script[i] or "ro.build.product" in edify.script[i]):
            edify.script[i] = edify.script[i].replace(" ||", ' || getprop("ro.build.product") == "m86" || getprop("ro.product.device") == "m86" || getprop("ro.build.product") == "m86" ||')
            return
...
def AddArgsForFormatSystem(info):
  edify = info.script
  for i in xrange(len(edify.script)):
    if "format(" in edify.script[i] and "/dev/block/platform/15570000.ufs/by-name/system" in edify.script[i]:
      edify.script[i] = 'format("ext4", "EMMC", "/dev/block/platform/15570000.ufs/by-name/system", "0", "/system");'
      return
...
def FullOTA_InstallRecovery(info):
  ...
  InstallImage("recovery.img", img_file, "/dev/block/platform/15570000.ufs/by-name/recovery", info)
...
def IncrementalOTA_InstallRecovery(info):
  try:
    ...
    if source_file != target_file:
      info.script.Print("Writing recovery")
      InstallImage("recovery.img", target_file, "/dev/block/platform/15570000.ufs/by-name/recovery", info)
    else:
